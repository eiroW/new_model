...
class Model(nn.Module):
	def __init__(self, task_gen, args):
		...
		log.info('Building RNN and output layers...')
		self.z_size = 128
		self.key_size = 256
		self.hidden_size = 512
		self.RNN = nn.RNN(self.key_size + 1, self.hidden_size, batch_first=True)
	...
	def forward(self, x_seq, device):
		...
		for t in range(x_seq.shape[1] + 1):
			...
			# Controller
			# RNN
			RNN_out, hidden = self.RNN(key_r, hidden)
			# Key output layers
			key_w = self.relu(self.key_w_out(RNN_out))
			# Gates
			g = self.sigmoid(self.g_out(RNN_out))
			# Task output layer
			y_pred_linear = self.y_out(RNN_out).squeeze()
